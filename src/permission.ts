import inquirer from 'inquirer';

export type ActionType = 'READ_FILE' | 'WRITE_FILE' | 'APPLY_DIFF' | 'EXECUTE_COMMAND' | 'COMPLETE';

export interface ActionRequest {
  type: ActionType;
  description: string;
  details?: any;
}

export class PermissionHandler {
  private autoApprove: boolean;

  constructor(autoApprove: boolean = false) {
    this.autoApprove = autoApprove;
  }

  async requestPermission(request: ActionRequest): Promise<boolean> {
    if (this.autoApprove) {
      console.log(`Auto-approving action: ${request.description}`);
      return true;
    }

    const { type, description, details } = request;
    
    const message = this.formatRequestMessage(type, description, details);
    const { confirm } = await inquirer.prompt([{
      type: 'confirm',
      name: 'confirm',
      message: `${message}\nProceed with this action?`,
      default: false
    }]);
    
    return confirm;
  }

  private formatRequestMessage(type: ActionType, description: string, details?: any): string {
    let message = `Action: ${description}`;
    
    switch (type) {
      case 'READ_FILE':
        message += `\nFile Path: ${details.path}`;
        break;
      case 'WRITE_FILE':
        message += `\nFile Path: ${details.path}\nContent Length: ${details.contentLength} characters`;
        break;
      case 'APPLY_DIFF':
        message += `\nFile Path: ${details.path}\nDiff: ${details.diffSummary}`;
        break;
      case 'EXECUTE_COMMAND':
        message += `\nCommand: ${details.command}`;
        if (details.cwd) message += `\nWorking Directory: ${details.cwd}`;
        break;
    }
    
    return message;
  }
}